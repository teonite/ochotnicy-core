...
class NotSignedAgreementState(AgreementState):
    def assert_can_upload(self):
    ...
    def get_next_state(self):
        if 'organization_signed_resource' in self.request.DATA:
            return AgreementStateFactory.SIGNED_BY_ORGANIZATION
        elif 'volunteer_signed_resource' in self.request.DATA:
            return AgreementStateFactory.SIGNED_BY_VOLUNTEER
        raise WrongRequestException("You have not given resource")
    ...
    def get_uploading_field_value(self):
        return 'organization_signed_resource' if self.get_next_state() is AgreementStateFactory.SIGNED_BY_ORGANIZATION \
            else 'volunteer_signed_resource'


class SignedByOrganizationAgreementState(AgreementState):
    def assert_can_upload(self):
        if 'organization_signed_resource' in self.request.DATA:
    ...
    def get_next_state(self):
        if 'volunteer_signed_resource' in self.request.DATA:
            return AgreementStateFactory.SIGNED_BY_BOTH
        raise WrongRequestException("You have not given resource")
...
class SignedByVolunteerAgreementState(AgreementState):
    def assert_can_upload(self):
        if 'volunteer_signed_resource' in self.request.DATA:
...
class SignedByBothAgreementState(AgreementState):
    def assert_can_upload(self):
...
class AgreementStateFactory:
    ...
    products = (
        (NOT_SIGNED, NotSignedAgreementState),
        (SIGNED_BY_ORGANIZATION, SignedByOrganizationAgreementState),
        (SIGNED_BY_VOLUNTEER, SignedByVolunteerAgreementState),
        (SIGNED_BY_BOTH, SignedByBothAgreementState)
    )
